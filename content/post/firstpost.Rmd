...
The winner then gets the **European Bayes League trophy**, typically consisting in a publication with discussion on JRSS-B. 
...
Enough with the babbling, my dad wants to know if the money spent on my education, and the fact that he hasn't seen any grandchild yet worth it. 
...
knitr::kable(Tab_1, caption = "Table 1. Predictions without first round of the semi-finals", 
             col.names =  colnames)
knitr::kable(Tab_3, caption = "Table 2. Predictions with first round of the semi-finals", 
             col.names =  colnames)
```

At this point we can say with some evidence that the model in terms of predicting Champion's League results is more or less lousy. A natural extension (that does not require hours of looking for covariates) is to include a random effect that accounts for phase effect. To get an estimate about the semi-finals we needed some data on the semi-finals, thus we used the updated dataset with the first round of the semi-finals. The results again are far from the truth; however it seems that by adding a phase effect, the home team gets an advantage. Just think to the Roma results: (4:1; 3:0) with Barcellona and (5:2, 4:2) with Liverpool. Given this results it seems reasonable that the phase effect turns out to be an home effect. Although far-fetched and marginally overinterpretation with the amount of data available, a possible explanation of this effect might be that as a team approaches the Champion's League final, fans tend to be more and more supportive and this support is better reflected on the Home team. 
...
colnames <- c("H-Team", "A-Team", "H-Goal<br/> Mean (sd)", "A-Goal<br/> Mean (sd)", "H-Team wins<br/> P (CI)", 
              "A-team wins<br/> P (CI)", "Tie<br/> P (CI)")
knitr::kable(Tab_2, caption = "Table 3. Predictions with first round of the semi-finals and phase effect", 
             col.names =  colnames)
```
...
knitr::kable(Tab_4,  col.names =  colnames, 
             caption = "Table 4. All possible prediction for the Champion's League winner")
```
